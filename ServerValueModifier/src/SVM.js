...
class MainSVM
{
	 ...
	 postDBLoad(container)
	{
		...
		Logger.log(`SVM 1.6.6 has initialized, The author of this mod is totally not lazy!`, "blue");
		...
		if (Config.Bots.EnableBots)
		{
						...
						for (let x in locations[i].base.BossLocationSpawn)
						{
							switch (locations[i].base.BossLocationSpawn[x].BossName)
							{
								...
								case "bossGluhar":
									...
									if (i == "tarkovstreets")
									{
										locations[i].base.BossLocationSpawn[x].BossChance = Config.Bots.AIChance.GlukharStreetsChance
									}
								break;
								case "bossKojaniy":
			...
			Bots.pmc.convertIntoPmcChance.assault.min = Config.Bots.AItoPMC.PMCtoScav;
			Bots.pmc.convertIntoPmcChance.cursedassault.min = Config.Bots.AItoPMC.PMCtoCursedScav;
			Bots.pmc.convertIntoPmcChance.pmcbot.min = Config.Bots.AItoPMC.PMCtoRaider;
			Bots.pmc.convertIntoPmcChance.exusec.min = Config.Bots.AItoPMC.ExusectoPMC;
			Bots.pmc.convertIntoPmcChance.marksman = [];
			Bots.pmc.convertIntoPmcChance.marksman.min = Config.Bots.AItoPMC.SnipertoPMC;

			Bots.pmc.convertIntoPmcChance.assault.max = Config.Bots.AItoPMC.PMCtoScav;
			Bots.pmc.convertIntoPmcChance.cursedassault.max = Config.Bots.AItoPMC.PMCtoCursedScav;
			Bots.pmc.convertIntoPmcChance.pmcbot.max = Config.Bots.AItoPMC.PMCtoRaider;
			Bots.pmc.convertIntoPmcChance.exusec.max = Config.Bots.AItoPMC.ExusectoPMC;
			Bots.pmc.convertIntoPmcChance.marksman.max = Config.Bots.AItoPMC.SnipertoPMC;
		...
		{
			if(Config.Traders.OpArmorRepair)
			{
			...
			for (let CurTrader in traders)
			{
				if (CurTrader !== "ragfair" && (CurTrader == "5a7c2eca46aef81a7ca2145d" || CurTrader == "5ac3b934156ae10c4430e83c" || CurTrader == "5c0647fdd443bc2504c2d371" || CurTrader == "54cb50c76803fa8b248b4571" || CurTrader == "54cb57776803fa99248b456e" || CurTrader == "579dc571d53a0658a154fbec" || CurTrader == "5935c25fb3acc3127c3d8cd9" || CurTrader == "58330581ace78e27b8b10cee"))
				{
					...
					{
						traders[CurTrader].base.loyaltyLevels[level].repair_price_coef *= Config.Traders.RepairMult;
					}
				}
			}
			Repair.priceMultiplier = Config.Traders.RepairMult;
			trader.fence.assortSize = Config.Traders.FenceAmountOnSale;
			trader.fence.discountOptions.assortSize = Config.Traders.FencePremiumAmountOnSale;
			trader.fence.maxPresetsPercent = Config.Traders.FencePresetcount
			trader.fence.presetPriceMult = Config.Traders.FencePresetmult
		...
		//The idea would be creating most values into arrays and run them this way rather making direct requests, but hey, at least code is readable
		if (BL.EnableAdvLoad)
		{
			// BL redirects
			const BEAR = BL.BearAdvancedLoadout;
			const USEC = BL.UsecAdvancedLoadout;
			const RAIDER = BL.RaiderAdvancedLoadout;
			const SCAV = BL.ScavAdvancedLoadout;
			const ROGUE = BL.RogueAdvancedLoadout// kinda overkill for 5 variables but made it to make code in BL section readable
			const AddKeys = [];
			const AddContainers = [];
			const CurUsec = "usec"
			...
			const bottypes = [
				CurBear, "assault","cursedassault",
				CurUsec, "pmcbot", "marksman"
			];
			...
			//Change generation values - i really need to automate this process.
			...
			Bot[CurBear].generation.items.drugs.min = BEAR.BearDrugsMin
			Bot[CurBear].generation.items.drugs.max = BEAR.BearDrugsMax
			Bot[CurBear].generation.items.stims.min = BEAR.BearStimsMin
			Bot[CurBear].generation.items.stims.max = BEAR.BearStimsMax
			...
			Bot[CurUsec].generation.items.drugs.min = USEC.UsecDrugsMin
			Bot[CurUsec].generation.items.drugs.max = USEC.UsecDrugsMax
			Bot[CurUsec].generation.items.stims.min = USEC.UsecStimsMin
			Bot[CurUsec].generation.items.stims.max = USEC.UsecStimsMax
			...
			Bot["assault"].generation.items.drugs.min = SCAV.ScavDrugsMin
			Bot["assault"].generation.items.drugs.max = SCAV.ScavDrugsMax
			Bot["assault"].generation.items.stims.min = SCAV.ScavStimsMin
			Bot["assault"].generation.items.stims.max = SCAV.ScavStimsMax
				
			Bot["cursedassault"].generation.items.healing.min = SCAV.ScavHealingMin
			Bot["cursedassault"].generation.items.healing.max = SCAV.ScavHealingMax
			Bot["cursedassault"].generation.items.looseLoot.min = SCAV.ScavLootMin
			Bot["cursedassault"].generation.items.looseLoot.max = SCAV.ScavLootMax
			Bot["cursedassault"].generation.items.magazines.min = SCAV.ScavMagazinesMin
			Bot["cursedassault"].generation.items.magazines.max = SCAV.ScavMagazinesMax
			Bot["cursedassault"].generation.items.grenades.min = SCAV.ScavGrenadesMin
			Bot["cursedassault"].generation.items.grenades.max = SCAV.ScavGrenadesMax
			Bot["cursedassault"].generation.items.drugs.min = SCAV.ScavDrugsMin
			Bot["cursedassault"].generation.items.drugs.max = SCAV.ScavDrugsMax
			Bot["cursedassault"].generation.items.stims.min = SCAV.ScavStimsMin
			Bot["cursedassault"].generation.items.stims.max = SCAV.ScavStimsMax
				
			Bot["marksman"].generation.items.healing.min = SCAV.ScavHealingMin
			Bot["marksman"].generation.items.healing.max = SCAV.ScavHealingMax
			Bot["marksman"].generation.items.looseLoot.min = SCAV.ScavLootMin
			Bot["marksman"].generation.items.looseLoot.max = SCAV.ScavLootMax
			Bot["marksman"].generation.items.magazines.min = SCAV.ScavMagazinesMin
			Bot["marksman"].generation.items.magazines.max = SCAV.ScavMagazinesMax
			Bot["marksman"].generation.items.grenades.min = SCAV.ScavGrenadesMin
			Bot["marksman"].generation.items.grenades.max = SCAV.ScavGrenadesMax
			Bot["marksman"].generation.items.drugs.min = SCAV.ScavDrugsMin
			Bot["marksman"].generation.items.drugs.max = SCAV.ScavDrugsMax
			Bot["marksman"].generation.items.stims.min = SCAV.ScavStimsMin
			Bot["marksman"].generation.items.stims.max = SCAV.ScavStimsMax
			...
			Bot["pmcbot"].generation.items.drugs.min = RAIDER.RaiderDrugsMin
			Bot["pmcbot"].generation.items.drugs.max = RAIDER.RaiderDrugsMax
			Bot["pmcbot"].generation.items.stims.min = RAIDER.RaiderStimsMin
			Bot["pmcbot"].generation.items.stims.max = RAIDER.RaiderStimsMax

			Bot["exusec"].generation.items.healing.min = ROGUE.RogueHealingMin
			Bot["exusec"].generation.items.healing.max = ROGUE.RogueHealingMax
			Bot["exusec"].generation.items.looseLoot.min = ROGUE.RogueLootMin
			Bot["exusec"].generation.items.looseLoot.max = ROGUE.RogueLootMax
			Bot["exusec"].generation.items.magazines.min = ROGUE.RogueMagazinesMin
			Bot["exusec"].generation.items.magazines.max = ROGUE.RogueMagazinesMax
			Bot["exusec"].generation.items.grenades.min = ROGUE.RogueGrenadesMin
			Bot["exusec"].generation.items.grenades.max = ROGUE.RogueGrenadesMax
			Bot["exusec"].generation.items.drugs.min = ROGUE.RogueDrugsMin
			Bot["exusec"].generation.items.drugs.max = ROGUE.RogueDrugsMax
			Bot["exusec"].generation.items.stims.min = ROGUE.RogueStimsMin
			Bot["exusec"].generation.items.stims.max = ROGUE.RogueStimsMax
			...
			Bot["cursedassault"].chances.equipment = SCAV.ScavEquipmentChanceList
			Bot["cursedassault"].chances.mods = SCAV.ScavAttachmentChanceList
				
			Bot["marksman"].chances.equipment = SCAV.ScavEquipmentChanceList
			Bot["marksman"].chances.mods = SCAV.ScavAttachmentChanceList
				
			Bot["exusec"].chances.equipment = ROGUE.RogueEquipmentChanceList
			Bot["exusec"].chances.mods = ROGUE.RogueAttachmentChanceList
		...
		if (Config.Quests.EnableQuests)
		{
			...
			Quest.repeatableQuests[0].questConfig.Completion.useWhitelist = Daily.DailyWhitelist
			Quest.repeatableQuests[0].questConfig.Completion.useBlacklist = Daily.DailyBlacklist
			...
			Quest.repeatableQuests[1].questConfig.Completion.useWhitelist = Weekly.WeeklyWhitelist;
			Quest.repeatableQuests[1].questConfig.Completion.useBlacklist = Weekly.WeeklyBlacklist;
