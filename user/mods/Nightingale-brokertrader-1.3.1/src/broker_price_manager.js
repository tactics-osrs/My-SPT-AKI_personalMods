...
class BrokerPriceManager {
    constructor(container) {
        this.supportedTraders = [];
        this._clientBrokerSellData = {};
    ...
    initializeCurrencyBuyRates() {
        ...
        const pkAssort = this.traderHelper.getTraderAssortsById(Traders_1.Traders.PEACEKEEPER);
        ...
        const skiAssort = this.traderHelper.getTraderAssortsById(Traders_1.Traders.SKIER);
    ...
    getItemTemplateRagfairPrice(itemTplId) {
        ...
        const validOffersForItemTpl = this.ragfairOfferService.getOffersOfType(itemTplId)?.filter(offer => {
            ...
            if (offer.user?.memberType === MemberCategory_1.MemberCategory.TRADER || // no trader offers
                ...
                config_json_1.default.ragfairIgnoreAttachments && this.presetHelper.hasPreset(firstItem._tpl) && offer.items.length === 1 || // only "operational" offers if config specifies
                !config_json_1.default.ragfairIgnoreAttachments && this.presetHelper.hasPreset(firstItem._tpl) && offer.items.length > 1 // only "not operational" offers if config specifies
            )
                return false;
...
BrokerPriceManager.brokerTraderId = base_json_1.default._id; // for flea trades
BrokerPriceManager.brokerTraderCurrencyExhangeId = `${base_json_1.default._id}-currency-exchange`; // for currency exchange
